...
am4core.ready(function() {
  ...
  var deathsColor = am4core.color("#ccc");
...
*/
  ...
  var container = am4core.create("map", am4core.Container);
  ...
  absolutePerCapitaSwitch.events.on("toggled", function() {
    ...
    updateLineChart();
    updateTotals(Math.round((covid_world_timeline.length - 1) * slider.start));
  })
  ...
  function updateColumnsFill() {
    columnSeries.active.columns.each(function(column) {
      if (column.dataItem.values.valueY.previousChange < 0) {
        ...
        //column.dataItem.values.valueY = 1;
      }
      else {
  ...
  function changeDataType(name) {
    ...
    filterSlider.startGrip.background.fill = colors[name];
    sizeSlider.startGrip.background.fill = colors[name];

    //change color hover list
    root.style.setProperty('--hoverColor', colors[name]);
  ...
  function selectCountry(mapPolygon) {
    ...
    // pass selected country to pie chart
    selectedCountry(mapPolygon.dataItem.dataContext);
  ...
  function setCountryData(countryIndex) {
    ...
    for (var i = 0; i < lineChart.data.length; i++) {
      ...
      console.log(populations[countryData.id])
      if (countryData) {
        if(perCapita){
            dataContext.recovered = numberFormatter.format(countryData.recovered / populations[countryData.id] * 1000000, '###.#');
            dataContext.confirmed = numberFormatter.format(countryData.confirmed / populations[countryData.id] * 1000000, '###.#');
            dataContext.deaths = numberFormatter.format(countryData.deaths / populations[countryData.id] * 1000000, '###.#');
            dataContext.active = numberFormatter.format((countryData.confirmed - countryData.recovered - countryData.deaths) / populations[countryData.id] * 1000000, '###.#');
  ...
  function updateTotals(index) {
    if (!isNaN(index)) {
      ...
      var date = new Date(di.date + " 00:00");
      currentDate = date;
  ...
  function populateCountries(list) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      $("<td>").appendTo(tr).data("areaid", area.id).html('<img "src=/flags/png/009-uganda.png" alt="" border="3" height="15" width="15"></img>' + area.name);
  ...
  function updateLineChart(){
        ...
        if(perCapita){
            dataContext.recovered = numberFormatter.format(di.recovered / populations[currentCountry] * 1000000, '###.#');
            dataContext.confirmed = numberFormatter.format(di.confirmed / populations[currentCountry] * 1000000, '###.#');
            dataContext.deaths = numberFormatter.format(di.deaths / populations[currentCountry] * 1000000, '###.#');
            dataContext.active = numberFormatter.format((di.confirmed - di.recovered - di.deaths) / populations[currentCountry] * 1000000, '###.#');
            valueAxis.min = undefined;
            valueAxis.max = undefined;
        }else{
